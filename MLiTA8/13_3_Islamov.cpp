// MLiTA8.cpp: определяет точку входа для консольного приложения.
//Исламов Яхья ПС-21
...
void TuneOffsetTable(vector<int>& dict, string obrazec)
{
	...
	{
		if (dict[(int)obrazec[i]] == obrazec.length()+1)
	...
	if (dict[(int)obrazec[obrazec.length() - 1]] == obrazec.length() + 1)
		dict[(int)obrazec[obrazec.length() - 1]] = obrazec.length();
}
...
int searchWordinLine(vector<letterInLine> queue, int index, string obrazec, vector<int> dict)
{
	...
	for (size_t i = obrazec.length() - 1; i >= 0; i--)
	{
		...
		{
			return dict[(int)queue[index].letter];
		}
		index--;
		if (i == 0)
			break;
